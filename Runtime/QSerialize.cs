                    ...
                    case TypeCode.Object:
                        {
                            ...
                            switch (typeInfo.state)
                            {
                                ...
                                case QTypeState.List:
                                    ...
                                    {
                                        writer.Write(SerializeType(item, typeInfo.ElementType));
                                    }
                                    break;
                                case QTypeState.Array:
                                    ...
                                    ForeachArray(array, 0, typeInfo.IndexArray, (indexArray) => { writer.Write(SerializeType(array.GetValue(indexArray), typeInfo.ElementType)); });
                                    break;
                            ...
                            case QTypeState.List:
                                {
                                    ...
                                    for (int i = 0; i < count; i++)
                                    {
                                        if (list.Count > i)
                                        {
                                            list[i] = DeserializeType(reader.ReadBytes(), typeInfo.ElementType, list[i]);
                                        }
                                        else
                                        {
                                            list.Add(DeserializeType(reader.ReadBytes(), typeInfo.ElementType));
                                        }
                            ...
                            case QTypeState.Array:
                                {
                                    ...
                                    ForeachArray(array, 0, typeInfo.IndexArray, (indexArray) =>
                                    {
                                        ...
                                        array.SetValue(DeserializeType(reader.ReadBytes(), typeInfo.ElementType, array.GetValue(indexArray)), indexArray);
                                    });
                                    return array;
                                }
                            case QTypeState.ISerialize:
                            ...
                            case QTypeState.Normal:
                                {
                                    ...
                                    for (int i = 0; i < memberCount; i++)
                                    {
                                        ...
                                        var tbytes = reader.ReadBytes();
                                        if (typeInfo.Members.ContainsKey(name))
                                        {
                                            var memeberInfo = typeInfo.Members[name];
                                            memeberInfo.Set.Invoke(obj, DeserializeType(tbytes, memeberInfo.Type, target != null ? memeberInfo.Get?.Invoke(target) : null));
                                        }
                                    }
                                    return obj;
                                }
